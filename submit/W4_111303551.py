...
def feature_engineering(df):
    ...
    # TODO 2.1: 計算總分
    df['總分'] = df[['國文', '英文', '數學','社會','自然']].sum(axis=1)
...
def filter_and_analyze_data(df):
    ...
    table={
        '國文': df['國文'].describe(include='all'),
        '英文': df['英文'].describe(include='all'),
        '數學': df['數學'].describe(include='all'),
        '社會': df['社會'].describe(include='all'),
        '自然': df['自然'].describe(include='all'),
        '總分': df['總分'].describe(include='all'),
        '平均': df['平均'].describe(include='all'),
    }
    ...
    # TODO 4.2: 找出總分最高的學生
    print("--- 總分最高的學生 ---")
    top_student = df[df['總分'] == df['總分'].max()][['姓名', '班級', '總分', '平均']]
    print(top_student)
