...
def load_and_explore_data(file_path):
    """任務一：讀取 CSV 並初步探索資料"""
    df = pd.read_csv("/workspaces/1141-ML-W4-Pandas/submit.csv", encoding='utf-8-sig')
...
def filter_and_analyze_data(df):
    ...
    math_failed = df[df['數學']<60][['姓名','班級','數學']]
    
    # TODO 3.2: 找出班級為 'A' 且英文 > 90 的學生
    high_A = df[(df['班級']=='A')&(df['英文']>90)][['姓名','班級','英文']]
...
def save_results(df, output_file_path):
    """任務五：儲存為 CSV"""
    df.to_csv('new_grades.csv',encoding='utf-8-sig',index=True)
